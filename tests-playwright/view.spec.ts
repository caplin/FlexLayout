...
test.describe("Maximize methods", () => {
    ...
    test("maximize tabset using max button", async ({ page }) => {
        ...
        await expect.poll(async () => (await findPath(page, "/ts1/t0").boundingBox())!.width).toBeGreaterThan(panelBefore!.width * 2);
        ...
        await expect.poll(async () => Math.round((await findPath(page, "/ts1/t0").boundingBox())!.width)).toBe(Math.round(panelBefore!.width));
    });
...
test.describe("Extended App", () => {
    ...
    test("onRenderTab", async ({ page }) => {
        const tabButton = findPath(page, "/ts1/tb0").locator(`.${CLASSES.FLEXLAYOUT__TAB_BUTTON_LEADING}`).locator("img");
    ...
    test("onRenderTab in border", async ({ page }) => {
        const borderButton = findPath(page, "/border/top/tb0").locator(`.${CLASSES.FLEXLAYOUT__BORDER_BUTTON_LEADING}`).locator("img");
    ...
    test("onRenderTabSet", async ({ page }) => {
        const toolbar = findPath(page, "/ts1/tabstrip").locator(`.${CLASSES.FLEXLAYOUT__TAB_TOOLBAR}`);
    ...
    test("onRenderTabSet sticky buttons", async ({ page }) => {
        const stickyToolbar = findPath(page, "/ts2/tabstrip").locator(`.${CLASSES.FLEXLAYOUT__TAB_TOOLBAR_STICKY_BUTTONS_CONTAINER}`);

        await expect(stickyToolbar.locator("img").nth(0)).toHaveAttribute("src", "images/add.svg");
    });

    test("onRenderTabSet for border", async ({ page }) => {
        const borderToolbar = findPath(page, "/border/top").locator(`.${CLASSES.FLEXLAYOUT__BORDER_TOOLBAR}`);
