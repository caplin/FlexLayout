...
export const useTabOverflow = (
  ...
  const updateHiddenTabsTimerRef = React.useRef<NodeJS.Timeout | undefined>(
    undefined,
  );
  ...
  const onScrollPointerDown = (event: React.PointerEvent<HTMLElement>) => {
    ...
    const miniScrollElem = miniScrollRef.current;
    if (!miniScrollElem) return;
    const r = miniScrollElem.getBoundingClientRect();
    if (orientation === Orientation.HORZ) {
