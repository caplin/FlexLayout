...
export const BorderTabSet = (props: IBorderTabSetProps) => {
  ...
  const selectedTabNode =
    selectedIndex !== -1
      ? (border.getChildren()[selectedIndex] as TabNode)
      : undefined;
  ...
  React.useEffect(() => {
    if (selectedTabNode && !isPinned) {
      const onBodyPointerDown = (e: PointerEvent) => {
        ...
        if (
          !border.getTabHeaderRect().contains(x, y) &&
  ...
  if (selectedTabNode !== undefined) {
    const pinTitle = selectedTabNode.isPinned()
      ? layout.i18nName(I18nLabel.Unpin_Tab)
      : layout.i18nName(I18nLabel.Pin_Tab);
    const pinIcon = selectedTabNode.isPinned()
      ? typeof icons.unpin === 'function'
        ? icons.unpin(selectedTabNode)
        : icons.unpin
      : typeof icons.pin === 'function'
        ? icons.pin(selectedTabNode)
        : icons.pin;
    const onPinClick = (event: React.MouseEvent<HTMLElement, MouseEvent>) => {
      layout.doAction(
        Actions.updateNodeAttributes(selectedTabNode.getId(), {
          pinned: !selectedTabNode.isPinned(),
        }),
      );
      event.stopPropagation();
    };
  ...
  if (leading) {
    leadingContainer = (
      <div className={cm(CLASSES.FLEXLAYOUT__BORDER_LEADING)}>{leading}</div>
    );
  }
