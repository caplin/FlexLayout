...
export const Splitter = (props: ISplitterProps) => {
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLElement>) => {
    ...
    const rect = new Rect(r.x - domRect.x, r.y - domRect.y, r.width, r.height);
    ...
    if (outlineDiv.current) {
      ...
      if (node.getOrientation() === Orientation.VERT) {
        outlineDiv.current!.style.top =
          getBoundPosition(y - clientRect.y - dragStartY.current) + 'px';
      } else {
        outlineDiv.current!.style.left =
          getBoundPosition(x - clientRect.x - dragStartX.current) + 'px';
      }
